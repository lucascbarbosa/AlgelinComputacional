...
def main():
    ...
    size = int(sqrt(len(matA)))
    matA = list(map(lambda x: int(x),matA))
    matA = np.array(matA).reshape((size,size))
    matB = input('Digite a matriz B linearizada com os valores separados por , : ').split(',')
    matB = list(map(lambda x: int(x),matB))
    matB = np.array(matB)
    if check_singularity(matA): print ('Erro. Há singularidade na matriz informada.') 
    else:
